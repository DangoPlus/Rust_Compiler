...
%token LE_OP GE_OP EQ_OP NE_OP AND OR NOR EQUALS WHILE
%token BREAK CASE CONST CONTINUE DEFAULT ELSE
%token FOR FUNC GO IF IMPORT NIL PRINT PRINTLN READ
%token RETURN STRING STRUCT SWITCH TYPE VAR
...
func_def
: func_expr type_specifier IDENTIFIER '(' para_list ')' compound_stat
{
	insert( $3, $2, "" );
}
| func_expr IDENTIFIER '(' para_list ')' compound_stat
{
	insert( $2, "", "" );
}
| func_expr type_specifier IDENTIFIER '('  ')' compound_stat
{
	insert( $3, $2, "" );
}
| func_expr IDENTIFIER '('  ')' compound_stat
{
	insert( $2, "", "" );
}
;
