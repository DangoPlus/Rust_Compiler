...
| multi_expr '*' unary_expr  {Trace("Reducing to multi_expr1\n");outer_value=outer_value+1;}
| multi_expr '/' unary_expr  {Trace("Reducing to multi_expr2\n");outer_value=outer_value+2;}
| multi_expr '%' unary_expr  {Trace("Reducing to multi_expr3\n");outer_value=outer_value+3;}
;
// add and minus
add_expr
...
//expression
expr
: assign_expr  {Trace("Reducing to expr1\n");inner_value++;}
| expr assign_expr  {Trace("Reducing to expr2\n");}
|'(' expr ')'  {Trace("Reducing to expr3\n");inner_value++;}
| '\"' expr '\"'  {Trace("Reducing to expr4\n");}
;
//choose a type
type_specifier
...
| LET MUT IDENTIFIER ':' type_specifier '=' type_value_decl ';'{
	...
	if (lookup($3, 0) >= 0)
	{
		...
		strcat(jasm_code_temp3, $5);
		strcat(jasm_code_temp3, " ");
		strcat(jasm_code_temp3, $3);
		strcat(jasm_code_temp3, " = ");
		strcat(jasm_code_temp3, $7);
		strcat(jasm_code_temp3, "\n");

		strcat(jasm_code_temp5, "\tfield static ");
		strcat(jasm_code_temp5, $5);
		strcat(jasm_code_temp5, " ");
		strcat(jasm_code_temp5, $3);
		strcat(jasm_code_temp5, " = ");
		strcat(jasm_code_temp5, $7);
		strcat(jasm_code_temp5, "\n");
	}
	check_scpoe = 0;
	{Trace("Reducing to declaration\n");}
